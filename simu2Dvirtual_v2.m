...
for cas = 2:2
  ...
  noise = 1; % Assurez-vous de définir la variable 'noise' avant de l'utiliser
  ...
  for phi = 1:pstep:180
    ...
    for theta = 1:5:180
       ...
       for q = 1:length(xvirt)
          phiadd = exp(1j * 2 * 1.4 * (xvirt(q) / lambda) + 1j * 2 * 1.7 * (yvirt(q) / lambda) + 1j * phivirt(q));
          solution(u, v) = solution(u, v) + exp(2 * 1j * pi * 2 / lambda * (xvirt(q) * sind(theta) * cosd(phi) + yvirt(q) * cosd(theta))) * phiadd;
